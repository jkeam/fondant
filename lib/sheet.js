 ...
 */
async function read(spreadsheetId, idField, auth, ranges) {
  const responses = [];
  ...
  for (const range of ranges) {
    ...
    const models = createModels(headers, rows, idField);
 ...
 */
function createModels(headers, rows, idField) {
  const models = [];
  for (const row of rows) {
    ...
    // if no id, then just generate one
    if (!model[idField]) {
      model.id = v4();
    }
    models.push(model);
